...
1. [Set up Environment and create inference Client](#1-set-up-environment-and-create-inference-client)
2. [Work with PDFs and other files](#2-work-with-pdfs-and-other-files)
3. [Structured outputs with Gemini 2.0 and Pydantic](#3-structured-outputs-with-gemini-20-and-pydantic)
4. [Extract Structured data from PDFs using Gemini 2.0](#4-extract-structured-data-from-pdfs-using-gemini-20)
...
Once you have the SDK and API key, you can create a client and define the model you are going to use the new Gemini 2.0 Flash model, which is available via [free tier](https://ai.google.dev/pricing#2\_0flash) with 1,500 request per day (at 2025-02-06). 
...
model\_id =  "gemini-2.0-flash" # or "gemini-2.0-flash-lite-preview-02-05"  , "gemini-2.0-pro-exp-02-05"
```
...
```bash
!wget -q -O https://storage.googleapis.com/generativeai-downloads/data/pdf\_structured\_outputs/handwriting\_form.pdf
!wget -q -O https://storage.googleapis.com/generativeai-downloads/data/pdf\_structured\_outputs/invoice.pdf
```
...
```python
invoice\_pdf = client.files.upload(file="invoice.pdf", config={'display\_name': 'invoice'})
```
...
```python
file\_size = client.models.count\_tokens(model=model\_id,contents=invoice\_pdf)
print(f'File: {invoice\_pdf.display\_name} equals to {file\_size.total\_tokens} tokens')
...
class Person(BaseModel):
    first\_name: str = Field(description="The first name of the person")
    last\_name: str = Field(description="The last name of the person")
    age: int = Field(description="The age of the person, if not provided please return 0")
    work\_topics: list[Topic] = Field(description="The fields of interest of the person, if not provided please return an empty list")
...
response = client.models.generate\_content(model=model\_id, contents=prompt, config={'response\_mime\_type': 'application/json', 'response\_schema': Person})
...
print(f"First name is {philipp.first\_name}")
```
...
```python
def extract\_structured\_data(file\_path: str, model: BaseModel):
    # Upload the file to the File API
    file = client.files.upload(file=file\_path, config={'display\_name': file\_path.split('/')[-1].split('.')[0]})
    # Generate a structured response using the Gemini API
    prompt = f"Extract the structured data from the following PDF file"
    response = client.models.generate\_content(model=model\_id, contents=[prompt, file], config={'response\_mime\_type': 'application/json', 'response\_schema': model})
...
- `handwriting\_form.pdf` : Extract the form number, plan start date and the plan liabilities beginning of the year and end of the year
...
![Invoice.pdf](https://storage.googleapis.com/generativeai-downloads/data/pdf\_structured\_outputs/invoice.png)
...
class Item(BaseModel):
    ...
    gross\_worth: float = Field(description="The gross worth of the item")

class Invoice(BaseModel):
    """Extract the invoice number, date and all list items with description, quantity and gross worth and the total gross worth."""
    invoice\_number: str = Field(description="The invoice number e.g. 1234567890")
    ...
    total\_gross\_worth: float = Field(description="The total gross worth of the invoice")


result = extract\_structured\_data("invoice.pdf", Invoice)
print(type(result))
print(f"Extracted Invoice: {result.invoice\_number} on {result.date} with total gross worth {result.total\_gross\_worth}")
for item in result.items:
    print(f"Item: {item.description} with quantity {item.quantity} and gross worth {item.gross\_worth}")
```
...
![handwriting\_form.pdf](https://storage.googleapis.com/generativeai-downloads/data/pdf\_structured\_outputs/handwriting\_form.jpg)
...
class Form(BaseModel):
    ...
    form\_number: str = Field(description="The Form Number")
    start\_date: str = Field(description="Effective Date")
    beginning\_of\_year: float = Field(description="The plan liabilities beginning of the year")
    end\_of\_year: float = Field(description="The plan liabilities end of the year")

result = extract\_structured\_data("handwriting\_form.pdf", Form)

print(f'Extracted Form Number: {result.form\_number} with start date {result.start\_date}. \nPlan liabilities beginning of the year {result.beginning\_of\_year} and end of the year {result.end\_of\_year}')
