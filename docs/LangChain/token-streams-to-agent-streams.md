...
![](https://cdn.prod.website-files.com/65c81e88c254bb0f97633a71/6a0f012b2ff304b9a169e892\_blue-57%20characters%20max.png)
...
![](https://cdn.prod.website-files.com/65c81e88c254bb0f97633a71/6a0f06708e386c66847d58fa\_Screenshot\_2026-05-20\_at\_9.26.47\_AM.png)
...
The new [streaming foundation](https://github.com/langchain-ai/agent-protocol/tree/main/streaming) starts with a common event envelope. Instead of opaque stream tuples, you get structured events labelled with what kind of work they describe and where in the agent tree they came from.
...
```python
run = await graph.astream\_events(
...
Streaming transformers are small classes that filter protocol events and push derived items into a named channel. The `ToolActivityTransformer` here watches the `messages` channel for tool-call starts and exposes the result as a `toolActivity` extension. See [Build your own projection](https://docs.langchain.com/oss/python/langgraph/event-streaming#build-your-own-projection) for the full pattern.
...
The [cookbook](https://github.com/langchain-ai/streaming-cookbook) includes a generative UI example where an agent emits declarative A2UI messages over a \`custom:a2ui\` channel. The React app subscribes to that extension and renders live interface surfaces as the agent produces them. This is the pattern we expect to become common: agents streaming product-specific state, not just assistant text.
...
![](https://cdn.prod.website-files.com/65c81e88c254bb0f97633a71/6a0f09898264d273630661f8\_multimodal.png)

Each page subscribes only to the media it needs, rendering text, images, and audio as they become available.

In the [cookbook's multimodal storybook demo](https://github.com/langchain-ai/streaming-cookbook/tree/main/typescript/multimodal), a graph generates a bedtime story, page images, audio narration, and video. The UI scopes media selectors to the graph nodes responsible for each page, so each page can render as soon as its assets arrive.

```typescript
const visualizer = useNodeRun(\`visualizer\_${index}\`);
...
- [`@langchain/react`](https://www.npmjs.com/package/@langchain/react)
- [`@langchain/vue`](https://www.npmjs.com/package/@langchain/vue)
- [`@langchain/svelte`](https://www.npmjs.com/package/@langchain/svelte)
- [`@langchain/angular`](https://www.npmjs.com/package/@langchain/angular)
