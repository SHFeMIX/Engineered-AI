...
with open("01\_kyoto\_poster.png", "wb") as f:
    f.write(base64.b64decode(images[0].data))
```

![01\_kyoto\_poster.png](https://www.philschmid.destatic/blog/nano-banana-2-interactions-api/01\_kyoto\_poster.jpg)


## 2. Text to Image with Google Search (Web Search)

Adding a `google\_search` tool makes the model search the web for factual information before generating.
...
vintage hand-drawn cartography style with warm earth tones. Title it 'KYOTO WALKING GUIDE'.""",
    tools=[
        {
            "type": "google\_search",
            "search\_types": ["web\_search"]
        }
    ]
)

images = [output for output in interaction.outputs if output.type == "image"]
with open("02\_kyoto\_map.png", "wb") as f:
    f.write(base64.b64decode(images[0].data))
```

![02\_kyoto\_map.jpg](https://www.philschmid.destatic/blog/nano-banana-2-interactions-api/02\_kyoto\_map.jpg)
...
Nano Banana 2 adds Image Search as a new search type alongside Web Search, controlled via the `search\_types` parameter. The model retrieves actual photographs from Google Image Search and uses them as visual references during generation.
...
the bottom.""",
    tools=[
        {
            "type": "google\_search",
            "search\_types": ["web\_search", "image\_search"]
        }
    ]
)

images = [output for output in interaction.outputs if output.type == "image"]
with open("03\_fushimi\_inari.png", "wb") as f:
    f.write(base64.b64decode(images[0].data))
```

![03\_fushimi\_inari.jpg](https://www.philschmid.destatic/blog/nano-banana-2-interactions-api/03\_fushimi\_inari.jpg)

You can also use `image\_search` on its own without `web\_search`:

```python
tools=[{"type": "google\_search", "search\_types": ["image\_search"]}]
```
...
image\_data = base64.b64encode(open("assets/headshot.png", "rb").read()).decode()
...
images = [output for output in interaction.outputs if output.type == "image"]
with open("04\_arashiyama\_portrait.png", "wb") as f:
    f.write(base64.b64decode(images[0].data))
```

![04\_arashiyama\_portrait.jpg](https://www.philschmid.destatic/blog/nano-banana-2-interactions-api/04\_arashiyama\_portrait.jpg)
...
- The `search\_types` parameter with `image\_search` requires the `gemini-3.1-flash-image-preview` model specifically.
...
- [Try in AI Studio](https://aistudio.google.com/prompts/new\_chat?model=gemini-3.1-flash-image-preview)
