---
title: "Better MoE model inference with warp decode · Cursor"
site: "Cursor"
published: "2026-04-06T12:00:00.000Z, 2026-04-06T12:00:00.000Z"
...
\<video width="1200" height="630"\>\<source src="https://ptht05hbb1ssoooe.public.blob.vercel-storage.com/assets/blog/og/blog-warp-decode-anim-20260406-075700-1200.mp4" type="video/mp4"\>\</video\>
...
Kernels that improve both performance and accuracy are rare, and warp decode is one of them. On Blackwell, it delivers a 1.84x throughput improvement while also improving accuracy with outputs 1.4x closer to a full FP32 reference. This speeds up the research and training pipeline for [Composer](https://cursor.com/blog/composer-2), letting us improve the model faster and ship new versions more often.
...
![](https://ptht05hbb1ssoooe.public.blob.vercel-storage.com/assets/blog/warp-decode-traditional-data-flow-r4.svg)
...
This warp independence lets warp decode run without any staging, handoffs, cross-warp sync points, or intermediate buffers. The entire MoE compute layer is compressed into two kernels, `moe\_gate\_up\_3d\_batched` and `moe\_down\_3d\_batched`.

![](https://ptht05hbb1ssoooe.public.blob.vercel-storage.com/assets/blog/warp-decode-data-flow-comparison-r4.svg)
...
![](https://ptht05hbb1ssoooe.public.blob.vercel-storage.com/assets/blog/warp-decode-architecture-r4.svg)
...
After all experts are processed, we reduce the 32 lane-local partial sums with a warp-level butterfly reduction using `\_\_shfl\_xor\_sync`. This compiles directly to the PTX `shfl.sync.bfly` instruction, which is a single hardware primitive that exchanges registers between lanes within the warp, bypassing shared memory entirely.
...
![](https://ptht05hbb1ssoooe.public.blob.vercel-storage.com/assets/blog/warp-decode-chart-throughput-gain-r4.svg)
...
![](https://ptht05hbb1ssoooe.public.blob.vercel-storage.com/assets/blog/warp-decode-chart-fp32-ground-truth-r3.svg)
...
![](https://ptht05hbb1ssoooe.public.blob.vercel-storage.com/assets/blog/warp-decode-chart-throughput-r3.svg)
