...
[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/google-gemini/cookbook/blob/main/quickstarts/Get\_started\_interactions\_api.ipynb)
...
interaction = client.interactions.create(
    ...
    system\_instruction="You are a helpful assistant.",
...
Use `previous\_interaction\_id` to continue a conversation.

Note: Only conversation history is preserved. Parameters like `tools` or `generation\_config` are interaction-scoped and must be re-declared if needed.
...
turn\_1 = client.interactions.create(
    model="gemini-3-flash-preview",
    input="My name is Alice and I am a software engineer."
)
print(f"Turn 1 ID: {turn\_1.id}")

turn\_2 = client.interactions.create(
    model="gemini-3-flash-preview",
    input="What is my job?",
    previous\_interaction\_id=turn\_1.id
)

print(turn\_2.outputs[-1].text)
...
turn\_2\_fork = client.interactions.create(
    model="gemini-3-flash-preview",
    input="What is my name?",
    previous\_interaction\_id=turn\_1.id
)

print(turn\_2\_fork.outputs[-1].text)
...
services. The API includes ready-to-use tools like Google Search, and lets you define custom tools as JSON schemas. The model decides when to call them based on the conversation:
...
text\_output = next((o for o in interaction.outputs if o.type == "text"), None)
if text\_output:
    print(text\_output.text)
```
...
for output in interaction.outputs:
    if output.type == "function\_call":
        ...
        # result = get\_weather(output.arguments
...
agent\_interaction = client.interactions.create(
...
# Poll for completion
while True:
    status\_check = client.interactions.get(agent\_interaction.id)
    print(f"Status: {status\_check.status}")
    
    if status\_check.status == "completed":
        print("\n--- Final Report ---\n")
        print(status\_check.outputs[-1].text)
        break
    elif status\_check.status in ["failed", "cancelled"]:
        print("Agent failed.")
        break
...
*   **[Thinking Models](https://ai.google.dev/api/interactions-api#thinking):** Configure `thinking\_level` for Gemini 2.5 and 3.0 models to handle complex reasoning.
