    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "\"$CLAUDE\_PROJECT\_DIR\"/.claude/hooks/check-tests.sh"
          }
        ]
      }
    ]
...
#!/bin/bash
INPUT=$(cat)
STOP\_HOOK\_ACTIVE=$(echo "$INPUT" | jq -r '.stop\_hook\_active // false')
...
When Claude finishes its turn, the Stop event fires. The script runs the test suite—if tests fail, it returns JSON with `decision: "block"` and a `reason`. Claude Code reads that, doesn't let Claude stop, and feeds the reason back into the conversation as instruction to keep working. The `stop\_hook\_active` guard at the top prevents infinite loops: if Claude is already continuing because of a previous stop-hook block, the script lets it exit.
