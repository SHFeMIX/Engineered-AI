---
title: "1.5x faster MoE training with custom MXFP8 kernels · Cursor"
site: "Cursor"
published: 2025-08-29T02:55:25.007Z, 2025-08-29T02:55:25.007Z
source: "https://cursor.com/blog/kernels"
domain: "cursor.com"
language: "en-US"
word_count: 4264
---
...
We recently began upgrading from Hopper GPUs (H100s) to Blackwell GPUs (B200s) and saw this as an opportunity to deeply optimize our training workloads. Profiling revealed that the main bottleneck was the Mixture-of-Experts (MoE) layer, implemented with [MegaBlocks](https://github.com/databricks/megablocks), which accounted for nearly 53% of forward-pass time and 27% of backward-pass time.

That’s why, over the past few weeks, **we rewrote the entire MoE layer from scratch at the GPU kernel level** with zero dependencies on any CUDA libraries. Instead, we used pure, good old CUDA and PTX, with a few bits of [ThunderKittens](https://github.com/HazyResearch/ThunderKittens) sprinkled in. As a result, we achieved a **3.5x improvement in MoE layer performance for both the forward and backward passes**, translating to a 1.5x end-to-end training speedup on Blackwell and a 2x speedup compared to our original Hopper setup. We believe our stack is faster than any combination of open-source alternatives available today.

![Figure 1. Relative speedups of MXFP8 MoE compared to BF16 (normalized to 1.0).](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-0.png&w=1920&q=70)

Figure 1. Relative speedups of MXFP8 MoE compared to BF16 (normalized to 1.0).

Most of this improvement came from transitioning from BF16 to [MXFP8](https://www.opencompute.org/documents/ocp-microscaling-formats-mx-v1-0-spec-final-pdf) which we achieved with nearly zero loss in training quality. But this also taught us that going low precision is easier said than done. If not done carefully, MXFP8 training may offer minimal performance improvement over BF16 due to various kernel overheads. The MXFP8 training recipe is also not widely shared, meaning you have to discover the right approach yourself.
...
Microscaling goes a step further by applying scaling to fine-grained sub-blocks of a tensor, rather than using a single scale factor for the entire tensor. The **Microscaling (MX) format** standardizes this approach by defining a set of low-precision, micro-scaled data formats. Full details are provided in its [specification](https://www.opencompute.org/documents/ocp-microscaling-formats-mx-v1-0-spec-final-pdf), which defines the following concrete MX-compliant formats:

![Table 1. Concrete MX-compliant formats.](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-1.png&w=1920&q=70)
...
![Figure 2. MXFP8 quantization example: Each 1x32 block shares a scale.](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-2.png&w=1920&q=70)
...
For example, in the well-known [DeepSeek V3 (DSV3)](https://arxiv.org/abs/2412.19437) technical report, the A matrix was scaled in blocks of size and the B matrix was scaled in blocks of size . This means the procedure is as follows:
...
This is no longer the case on Blackwell GPUs due to [tensor memory](https://docs.nvidia.com/cuda/parallel-thread-execution/#tensor-memory) (TMEM). TMEM is a new set of on-chip, SM-local memory added on Blackwell GPUs. Unlike Hopper GPUs, where the results of tensor core matrix multiplies accumulate directly in registers, Blackwell GPUs accumulate them in TMEM (via `tcgen05.mma` instruction). To perform custom arithmetic on the accumulators, you must transfer the results from TMEM to registers, process them with CUDA cores, write them back to TMEM, and then wait for all of these instructions to complete because the data movements are asynchronous. Although TMEM is faster to access than shared memory (SMEM), this still kills tensor core occupancy.

![Figure 3. Gantt chart taken from our custom Blackwell attention kernel. First row shows the tensor core activity (QKᵀ). Second row shows the CUDA core activity (TMEM → registers, then softmax). TMEM → register latency causes the tensor core pipeline bubble.](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-3.png&w=1920&q=70)
...
![Figure 4. BF16 vs MXFP8 Training Loss over 10k steps: nearly indistinguishable.](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-4.png&w=1920&q=70)

Figure 4. BF16 vs MXFP8 Training Loss over 10k steps: nearly indistinguishable.

![Figure 5. BF16 vs MXFP8 Training Loss (9k–10k steps) with a specific data point at step 9832.](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-5.png&w=1920&q=70)
...
![Figure 6. Simplified TMEM allocation: accumulator region plus 5 slots each for A and B scales](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-6.png&w=1920&q=70)

Figure 6. Simplified TMEM allocation: accumulator region plus 5 slots each for A and B scales

![Figure 7. Simplified SMEM allocation: 5 slots reserved for input tiles and scales](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-7.png&w=1920&q=70)
...
![Figure 8. Simplified MXFP8 Matrix Multiplication Pipeline](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-8.png&w=1920&q=70)
...
Maintaining high L2 cache utilization is critical. In our benchmarks, inefficient HBM access patterns could reduce performance by nearly 50% in grouped matrix multiplication kernels. To address this, we applied **supergrouping**, [a heuristic from ThunderKittens kernels](https://github.com/HazyResearch/ThunderKittens/blob/6c27e28c8115d1839d9eeeb530913c184a75fc87/kernels/matmul/FP8/matmul.cu#L37) that maximizes L2 reuse by ensuring that the region of the output matrix computed by all 148 SMs at any given time is as square as possible. You can learn more about this by reading the linked kernel code above.
...
- Produces the scale matrix [in the exact layout](https://docs.nvidia.com/cuda/parallel-thread-execution/#tcgen05-mma-scale-factor-a-layout-1x) [expected by](https://docs.nvidia.com/cuda/parallel-thread-execution/#tcgen05-mma-scale-factor-b-layout-1x) `tcgen05.mma`, allowing our MXFP8 kernel to execute HBM → SMEM → TMEM data flow without intermediate transformations.
...
![Figure 9. End-to-end training TPS per GPU (internal model).](https://cursor.com/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fkernels-9.png&w=1920&q=70)
...
If you are interested in writing and optimizing high-performance kernels, training large coding models, or our work broadly, we’d love to hear from you. Reach out to us at [hiring@cursor.com](mailto:hiring@cursor.com).
