...
Slack threads and thread replies are identified by unique thread\_ts which are required to fetch thread replies. and other IDs ( channel\_id, user\_id ) can be retrieved from a “detailed” tool response to enable further tool calls that require these. “concise” tool responses return only thread content and exclude IDs. In this example, we use ~⅓ of the tokens with tool responses.
