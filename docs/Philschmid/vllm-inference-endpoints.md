...
We can deploy the model in just [a few clicks from the UI](https://ui.endpoints.huggingface.co/new), or take advantage of the `huggingface\_hub` Python library to programmatically create and manage Inference Endpoints. If you are using the UI, you have to select `custom` container type in the advanced configuration and add `philschmi/vllm-hf-inference-endpoints` as the container image. The UI is a great way to get started, but we currently do not support customizing the environment variables from the UI. We will show you how to do this using the `huggingface\_hub` Python library.

![custom container](https://www.philschmid.de/static/blog/vllm-inference-endpoints/custom-container.jpg)

We are going to use the `huggingface\_hub` library to programmatically create and manage Inference Endpoints. First, we need to install the library:
...
```bash
huggingface-cli login --token YOUR\_API\_TOKEN
```

Now we can create the Inference Endpoint using the `create\_inference\_endpoint` function. We will deploy `meta-llama/Meta-Llama-3-8B-Instruct` on a single A10G instance with a `MAX\_MODEL\_LEN` to 8192 to avoid out of memory errors.
...
endpoint = create\_inference\_endpoint(
    ...
    region="us-east-1",
    type="protected",
    instance\_type="g5.2xlarge",
    instance\_size="medium",
    custom\_image={
        "health\_route": "/health",
        "env": { "MAX\_MODEL\_LEN": "8192" },
...
\_Note: The UI widgets are currently not working for custom container images (sending a request to "/" and not "v1/completion7/chat"). You will receive a 404 if you try it. We are working on fixing/adding this. In the meantime you need to curl or use the OpenAI SDK to interact with the model.\_
...
\_Note: We have to use `/repository` as the model since thats where Inference Endpoints mounts the model.\_

```python
from huggingface\_hub import HfFolder
from openai import OpenAI

ENDPOINT\_URL = endpoint.url + "/v1/" # if endpoint object is not available check the UI 
API\_KEY = HfFolder.get\_token()

# initialize the client but point it to TGI
client = OpenAI(base\_url=ENDPOINT\_URL, api\_key=API\_KEY)
...
In this blog post, we showed you how to deploy open LLMS with vLLM on Hugging Face Inference Endpoints using the custom container image. We used the `huggingface\_hub` Python library to programmatically create and manage Inference Endpoints. We also showed you how to test the Inference Endpoint using the OpenAI SDK. We hope this blog post helps you deploy your LLMs on Hugging Face Inference Endpoints.
